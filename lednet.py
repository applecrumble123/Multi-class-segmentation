...
class APN_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        x3= interpolate(x3, size=((h+3) // 4, (w+3) // 4), mode="bilinear", align_corners=True)	
        x2 = self.conv2(x2)
        x = x2 + x3
...
class Decoder (nn.Module):
    def __init__(self, num_classes):
        ...
        self.apn = APN_Module(in_ch=128,out_ch=8)
    ...
    def forward(self, input):
        ...
        out = interpolate(output, size=(720, 960), mode="bilinear", align_corners=True)
